...
function directIncomeReport() {
  ...
  const fetchRoyaltyHistory = async () => {
    try {
      const res = await fetchData({ url: '/api/v1/user/profile/commissiondatail' });
      if (res?.data) {
        setHistory(res.data);
        ...
        <CTable hover bordered className="text-nowrap">
          <CTableHead>
            <CTableRow>
              ...
              <CTableHeaderCell>SLSP</CTableHeaderCell>
              <CTableHeaderCell>SRSP</CTableHeaderCell>
          ...
          <CTableBody>
            {currentItems.map((item, index) => (
              <CTableRow key={index}>
                ...
                <CTableDataCell>{item?.paymenttype || "N/A"}</CTableDataCell>
                
                <CTableDataCell>{item?.slsp || "N/A"}</CTableDataCell>
                <CTableDataCell>{item?.srsp || "N/A"}</CTableDataCell>
                <CTableDataCell>{new Date(item.createdAt).toLocaleDateString() || "N/A"}</CTableDataCell>
              </CTableRow>
            ))}
          </CTableBody>
        </CTable>
